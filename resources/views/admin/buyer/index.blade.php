...
<div class="content">
    ...
    <div class="container-fluid">
        <div class="float-right mt-3">
            <a href="{{ route('admin.buyers.create') }}" class="btn btn-success">Add Buyer</a>
        </div>
        <div class="filter mt-3 mb-2" style="clear: both">
            <form action="{{ route('admin.buyers.index') }}">
                <div class="row">
                    <div class="col-sm-3">
                        <div class="form-group">
                            <label class="form-label">Buyer name</label>
                            <input type="text" class="form-control" name="filter_name" value="{{ request()->get('filter_name') }}" placeholder="Buyer name">
                        </div>
                    </div>
                    <div class="col-sm-3">
                        <div class="form-group">
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <a href="{{ route('admin.buyers.index') }}" class="btn btn-dark ms-1 mt30">Reset</a>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <div class="table-responsive">
            <table class="table-panel table table-bordered table-centered mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>#{{ $buyer->id }}</td>
                        <td><a href="{{ route('admin.buyers.show', $buyer->id) }}">{{ $buyer->firstname }} {{ $buyer->lastname }}</a></td>
                        <td>{{ $buyer->email }}</td>
                        <td>{{ $buyer->phone }}</td>
                        <td>@if($buyer->status) Enable @else Disable @endif</td>
                        <td class="text-center" style="min-width: 120px">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="{{ route('admin.buyers.edit', $buyer->id ) }}">Edit</a>
                                <a class="dropdown-item" href="{{ route('admin.buyers.show', $buyer->id) }}">View</a>
                                <a class="dropdown-item" onclick="confirmDelete({{ $buyer->id }})" >Delete</a>
                            </div>

                            <form id='delete-form{{ $buyer->id }}'
                                action='{{ route('admin.buyers.destroy', $buyer->id) }}'
                                method='POST'>
        ...
        <div class="col-12 bordertop paginate" style="float: right;">
            {{ $buyers->appends(request()->query())->links('pagination::bootstrap-4') }}
        </div>
    </div>
</div>
